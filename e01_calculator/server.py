...
class CalculatorHandler(BaseHTTPRequestHandler):
    server_version = "E01LocalCalculator/0.1.2a1"
    sys_version = ""
    receipt_writer = None  # Compatibility for direct handler construction in tests.
    ...
    def do_GET(self):
        ...
        if self.path == f"/{API_VERSION}/health":
            self._send(200, {"status": "ok", "api": API_VERSION})
        else:
    ...
    def do_POST(self):
        ...
        try:
            writer = getattr(self.server, "receipt_writer", None) or self.receipt_writer
            if writer is None:
                raise OSError("no receipt sink")
...
class CalculatorHTTPServer(ThreadingHTTPServer):
    ...
    def __init__(self, address, writer):
        ...
        self.receipt_writer = writer
        self._workers = threading.BoundedSemaphore(MAX_WORKERS)
        super().__init__(address, CalculatorHandler)
...
def create_server(port, receipt_path, host="127.0.0.1"):
    if host != "127.0.0.1":
    ...
    return CalculatorHTTPServer((host, port), ReceiptWriter(receipt_path))


def serve(port, receipt_path, host="127.0.0.1"):
    httpd = create_server(port, receipt_path, host)
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="E01 local calculator")
    parser.add_argument("--port", type=int, default=0)
    parser.add_argument("--receipts", default="audit/receipts.jsonl")
    args = parser.parse_args(argv)
    serve(args.port, args.receipts)
