...
def _scalar(value):
    ...
    if isinstance(value, str):
        if len(value) > 65536:
            raise ExpressionError("condition scalar exceeds the size limit")
        return value
    if type(value) in (int, float) and (not isinstance(value, float) or math.isfinite(value)):
        try:
            result = str(value)
        except ValueError as exc:
            raise ExpressionError("condition number exceeds the size limit") from exc
        if len(result) > 4096:
            raise ExpressionError("condition number exceeds the size limit")
        return result
